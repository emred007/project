...
import type { Order, OrderItem, Address, OrderStatus, Product } from '@/types'
...
export const useOrderStore = defineStore('order', () => {
  ...
  async function createOrder(data: {
    ...
    try {
      ...
      for (const item of cartStore.items) {
        if (item.product) {
          await supabase
            .from('products')
            .update({
              stock: Math.max(0, (item.product as Product).stock - item.quantity),
            })
      ...
      error.value = e instanceof Error ? e.message : 'Sipariş oluşturulamadı'
      return null
    } finally {
      loading.value = false
    }
