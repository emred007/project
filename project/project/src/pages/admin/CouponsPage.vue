...
async function save() {
  if (!form.value.code || form.value.value <= 0) return
  const data = {
    ...form.value,
  ...
  if (editingCoupon.value) { await adminStore.updateCoupon(editingCoupon.value.id, data) } else { await adminStore.createCoupon(data) }
