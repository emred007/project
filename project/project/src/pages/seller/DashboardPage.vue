...
onMounted(async () => {
  ...
  const [productsCount, pendingCount, ordersData] = await Promise.all([
    ...
    supabase.from('orders').select('items, created_at, order_number'),
  ])
  ...
  stats.value = {
    ...
    monthlyEarnings: Math.floor(Math.random() * 10000) + 1000,
      ...
      <v-card flat>
        ...
        <v-card-text>
          <v-list density="compact">
            <v-list-item v-for="order in recentOrders" :key="order.order_number" class="px-0">
          ...
          <v-card v-if="!recentOrders.length" flat class="text-center py-8">
