...
async function changeRole(userId: string, newRole: 'user' | 'seller' | 'admin') {
  const ok = await adminStore.updateUserRole(userId, newRole)
  if (ok) {
    toast.success('Kullanıcı rolü güncellendi')
    await adminStore.fetchAllUsers()
  } else {
    toast.error(adminStore.error || 'Rol güncellenemedi')
  }
}

function confirmDelete(user: { id: string; full_name?: string; username?: string; email: string }) {
  userToDelete.value = {
    id: user.id,
...
async function deleteUser() {
  ...
  const ok = await adminStore.deleteUser(userToDelete.value.id)
  if (ok) {
    toast.success('Kullanıcı silindi')
    deleteDialog.value = false
    userToDelete.value = null
  } else {
    toast.error(adminStore.error || 'Kullanıcı silinemedi')
  }
}
  ...
  <v-card flat>
            ...
            <td>
              <v-menu>
                ...
                <v-list density="compact">
                  ...
                  <v-divider v-if="!isSelf(user.id)" />
                  <v-list-item v-if="!isSelf(user.id)" class="text-error" @click="confirmDelete(user)">
                    <v-list-item-title>Kullanıcıyı Sil</v-list-item-title>
                  </v-list-item>
                </v-list>
              </v-menu>
            </td>
    ...
    <v-dialog v-model="deleteDialog" max-width="420">
      <v-card>
        ...
        <v-card-text>
          <strong>{{ userToDelete?.name }}</strong> kullanıcısını kalıcı olarak silmek istediğinize emin misiniz?
          Bu işlem geri alınamaz.
        </v-card-text>
        <v-card-actions>
          <v-spacer />
          <v-btn variant="text" @click="deleteDialog = false">İptal</v-btn>
          <v-btn color="error" :loading="adminStore.loading" @click="deleteUser">Sil</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </v-card>
