...
function onPhoneInput(value: string) {
  phone.value = formatTurkishPhoneInput(value)
}

async function handleRegister() {
  if (!passwordsMatch.value) return
  if (phone.value.trim() && !isValidTurkishPhone(phone.value)) return
  ...
  const success = await authStore.register({
    ...
    phone: phone.value.trim() ? formatTurkishPhoneInput(phone.value) : undefined,
  })
  loading.value = false
  ...
  if (success) {
    ...
    } else if (authStore.registrationNotice) {
        ...
        <v-card class="pa-6 pa-sm-8" color="surface">
          ...
          <v-form @submit.prevent="handleRegister">
            ...
            <v-text-field
              ...
              autocomplete="tel"
              maxlength="11"
              hint="11 hane, 05 ile başlamalı (opsiyonel)"
              :rules="[(v) => !v || isValidTurkishPhone(v) || '05 ile başlayan 11 haneli numara girin']"
              class="mb-3"
            ...
            <v-alert
              v-if="authStore.registrationNotice"
              type="info"
