...
import { findDemoCoupon } from '@/data/demoCoupons'
import {
...
export const useCartStore = defineStore('cart', () => {
  ...
  const discount = computed(() => {
    ...
    return calculateCouponDiscount(appliedCoupon.value, subtotal.value)
  })
    ...
    try {
      ...
      for (const item of items.value) {
        ...
        const { data: product } = await supabase
          ...
          .single()
        if (product) item.product = product as Product
      }
    } catch (e) {
  ...
  async function applyCoupon(code: string) {
    ...
    try {
      let dbCoupon = await fetchCouponByCode(normalized)

      if (!dbCoupon) {
        const demoCoupon = findDemoCoupon(normalized)
        if (demoCoupon) {
          dbCoupon = demoCoupon
        }
      }
      ...
      const result = validateCouponForCart(dbCoupon, subtotal.value)
      if (result.ok === false) {
